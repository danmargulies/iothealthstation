...
def calc_hr_and_spo2(ir_data, red_data):
    ...
    for k in range(exact_ir_valley_locs_count - 1):
        ...
        if ir_valley_locs[k + 1] - ir_valley_locs[k] > 3:
            ...
            red_ac = int(
                (red_data[ir_valley_locs[k + 1]] - red_data[ir_valley_locs[k]]) * (red_dc_max_index - ir_valley_locs[k])
            )
...
def remove_close_peaks(n_peaks, ir_valley_locs, x, min_dist):
    ...
    while i < n_peaks:
        ...
        while j < old_n_peaks:
            n_dist = (
                (sorted_indices[j] - sorted_indices[i]) if i != -1 else (sorted_indices[j] + 1)
